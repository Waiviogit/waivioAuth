...
describe( 'auth', async () => {
    ...
    describe( 'socialAuth', async () => {
        ...
        it( 'should create user with post locales if it exists', async () => {
            ...
            const { user } = await AuthenticationModule.Auth.socialAuth( { userName, pickSocialFields, alias, provider, avatar, id, postLocales } );
        ...
        it( 'check sign up without user name', async () => {
            sinon.stub( OperationsHelper, 'transportAction' ).returns( Promise.resolve( { success: true } ) );
            const { user, session, message } = await AuthenticationModule.Auth.socialAuth( { userName: null, pickSocialFields, alias, provider, avatar, id } );
