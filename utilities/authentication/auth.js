...
exports.socialAuth = async ({
  ...
  if (!userBySocial && userName) {
    ...
    const { user, message } = await signUpSocial({
      userName, alias, avatar, provider, id, postLocales, nightMode, email,
    });
    if (user) {
      signUpRequest({
        userName, alias, provider, avatar, postLocales, id, nightMode, email,
      });
      return { user };
    }
    return { message };
