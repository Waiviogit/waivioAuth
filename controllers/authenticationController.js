...
const socialSignIn = async (req, res, next) => {
  const { validation_error } = validators.validate(Object.assign(req.body, { nightMode: req.headers.nightmode }), validators.authentication.socialAuthShcema);
...
const validateAuthToken = async (req, res) => render.success(res, validateAuthTokenView({ user: req.user }));
...
const beaxySignIn = async (req, res) => {
  const { validation_error, params } = validators.validate(Object.assign(req.body, { nightMode: req.headers.nightmode }), validators.authentication.socialBeaxySchema);
